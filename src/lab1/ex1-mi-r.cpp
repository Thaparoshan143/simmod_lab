...
// this is similar to ex-1, but here, the rate (dCx) is also plotted & we inject the small con. in reactant after certain time..
// warning: this implementation contain multiple con. injection at given time, if single injection required use another version
int main() {
    ...
    std::vector<pType> dC1t {};
    std::vector<pType> dC2t {};
    std::vector<pType> dC3t {};
    std::vector<pType> vtime {};

    constexpr pType injCon[] = { 1, 4, 1 }; // amount to inject
    constexpr pType injTime[] = { 0.4, 0.1, 0.5 }; // time at which con. is injected, range(0, 1), 0-start, 1-end
    constexpr uint injCount = sizeof(injCon) / sizeof(pType);

    static_assert(sizeof(injCon) == sizeof(injTime), "Both conc. and time should have same length");
    ...
    for (uint i=1;i<iter;++i)
    {
        ...
        // check for conc. injection (only reactant is injected.)
        auto ind = ArrContains<pType>(injTime, injCount, pType(i)/iter);
        if (ind > -1)
        {
            std::cout << "Injecting now of ind: " << ind << std::endl;
            C1 += injCon[ind];
            C2 += injCon[ind];
        }
    }
    ...
    // rate 
    subplot(4, 2, 4);
    plot(vtime, dC1t, "-")->line_width(LINE_WIDTH);
    xlabel("time");
    ylabel("rate");
    title("Reactant 1 (A)");

    subplot(4, 2, 5);
    plot(vtime, dC2t, "-")->line_width(LINE_WIDTH);
    xlabel("time");
    ylabel("rate");
    title("Reactant 2 (B)");
    
    subplot(4, 2, 6);
    plot(vtime, dC3t, "-")->line_width(LINE_WIDTH);
    xlabel("time");
    ylabel("rate");
    title("Product 1 (C)");
